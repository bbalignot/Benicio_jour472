...
Tweet: There are 121 census tracts below the #baltimore household median income. That is 60.5% of all neighborhoods fall below the median income.
...
baltcity_race %>%
  ...
  mutate (ratio = x2020_white/x2020_black, na.rm=TRUE) %>%
  arrange(desc(ratio))
...
baltcity_race %>%
  ...
  mutate (pct_white = x2020_white/sum(baltcity_race$x2020_white, na.rm=TRUE), na.rm=TRUE) %>%
